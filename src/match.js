...
export function match_replace_top(pattern_block, replace_block, workspace_blocks_json)
...
function match_replace_block(json_block_holder, pattern_block_json, replace_block)
{
  ...
  if (found && replace_block)
  {
    ...
    for (var j=0;j<variables.length;j++)
    {
      ...
      var variable = variables[j];
      replace_block_json = replace_variable(replace_block_json, variable.name, variable.new_value, variable.loc)
    }
...
function match(pattern, block) {
    if (pattern.type == "match_tree")
    {
      // no need to store location information for a tree
      return [true, [{"name": pattern.fields["TREE"], "new_value": block}]] 
    }
    if (!block)
    {
        return [false, []];
    ...
    if (pattern.inputs)
    {
      for (const [key,pattern_child] of Object.entries(pattern.inputs))
      {
          ...
          [found, child_save_variables] = match(pattern_child.block, input_block)
          if (found) {
  ...
  function replace_variable(replace_block, variable, new_value, loc)
...
// if variable is from field (by default $value than new_value is also a value)
{
  ...
  {
    replace_block.next.block = replace_variable(replace_block.next.block, variable, new_value, loc)
  }
  if (replace_block && replace_block.inputs)
  {
    for (const [key, input_block] of Object.entries(replace_block.inputs)) 
    {
      input_block.block = replace_variable(input_block.block, variable, new_value, loc)
    }
  }
  return replace_block;
}